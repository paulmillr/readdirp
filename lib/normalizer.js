...
function normalizeFilter(filter) {
  ...
  if (typeof filter === "string") {
    return entryInfo => isMatch(entryInfo.basename, filter.trim());
  }
  ...
  if (Array.isArray(filter)) {
    ...
    return isNegated(trimedFilter)
      ? entryInfo => trimedFilter.every(f => isMatch(entryInfo.basename, f))
      : entryInfo => trimedFilter.some(f => isMatch(entryInfo.basename, f));
  }
}
