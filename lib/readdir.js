    ...
    function process(currentDir, entries, callProcessed) {
        ...
        if (entries.length === 0) {
            ...
            entries.forEach(function (entry) { 
                ...
                var workingPath = path.join(currentDir, entry),
                    relPath  = path.join(dir, entry);

                fs.stat(workingPath, function (err, stat) {
                    if (err) {
                        errors.push(err);
                    } else {
                        entryInfos.push({
                            parentDir        :  dir,         // relative to root
                            workingParentDir :  currentDir,  // relative to working directory
                            name             :  entry,
                            path             :  relPath,     // relative to root
                            workingPath      :  workingPath, // relative to working directory
                            stat             :  stat
                        });
                    }
                    processed++;
            ...
            process(currentDir, entries, function(entryInfos) {
                ...
                if(pendingSubdirs === 0 || depth === opts.depth) {
                    ...
                    subdirs.forEach(function (subdir) {
                        readdirRec(subdir.workingPath, depth + 1, function () {
