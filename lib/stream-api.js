...
class ReaddirpReadable extends Readable {
  ...
  stat(path, callback) {
    return this._options.lstat ? fs.lstat(path, callback) : fs.stat(path, callback);
  }
  ...
  _prepeareToReading(currentDirectory, files, depth, done) {
    ...
    fs.realpath(currentDirectory, (error, directoryPath) => {
      ...
      const relativeDirectoryPath = sysPath.relative(
        this._options.root,
        directoryPath
      );
  ...
  _processEntry(basename, fullDirectoryPath, relativeDirectoryPath, depth, done) {
    if (this._destroyed) {
      return;
    }
    const fullPath = sysPath.join(fullDirectoryPath, basename);
    const relativeEntryPath = sysPath.join(relativeDirectoryPath, basename);

    this.stat(fullPath, (error, stat) => {
      if (error) {
        done();
        this._handleError(error);
        return;
