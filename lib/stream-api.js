...
class ReaddirpReadable extends Readable {
  static get defaultOptions() {
    return {
      root: ".",
      fileFilter: (entry) => true,
      directoryFilter: (entry) => true,
      entryType: "files",
  ...
  stat(file, cb) {
    return this._options.lstat ? fs.lstat(file, cb) : fs.stat(file, cb);
  }
  ...
  isFile(entryInformation) {
    const isCorrectType =
      (this._options.entryType === "all" && !entryInformation.stat.isDirectory()) ||
  ...
  willIncludeDir() {
    return ['directories', 'both', 'all'].includes(this._options.entryType);
  }

  willIncludeFile() {
    return ['files', 'both', 'all'].includes(this._options.entryType);
  }
   ...
   */
  _processEntry(name, fullDirectoryPath, relativeDirectoryPath, depth, callback) {
    if (this._destroyed) {
      return;
    }
  ...
  _decideToPushEntry(entryInformation, depth, callback) {
    ...
    const isPickedDirectory = isSubDirectory && this.willIncludeDir();
    const isPickedFile = this.isFile(entryInformation) && this.willIncludeFile();
